...
const Root = ({ single = false, ...props }: BoxProps & { single?: boolean }) => (
    <Box
        {...props}
        sx={[
            {
                ...
                maxWidth: single ? '500px' : 'unset',
                margin: single ? 'auto' : 'unset',
...
const Main = (props: BoxProps) => (
    <Box
        ...
        className="Main"
        {...props}
        sx={[{ p: 2, minHeight: `calc(100vh - 158px)`, boxSizing: 'border-box', maxWidth: '700px', width: '100%', marginX: 'auto' }, ...(Array.isArray(props.sx) ? props.sx : [props.sx])]}
    />
);
